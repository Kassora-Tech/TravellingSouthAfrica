    ...
    <Card className="flex flex-col overflow-hidden hover:shadow-lg transition-shadow">
      {listing.imageUrls && listing.imageUrls.length > 0 ? (
        <div className="relative w-full h-48">
          <Image src={listing.imageUrls[0]} alt={listing.name} fill className="object-cover" />
        </div>
      ) : (
      ...
      <CardContent className="p-4 flex flex-col flex-1 space-y-3">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="bg-secondary px-2 py-0.5 rounded-full text-xs">{listing.category}</span>
            {listing.townSlug && (
        ...
        <div className="space-y-1 pt-2 border-t">
          {listing.contactPhone && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <a href={`tel:${listing.contactPhone}`} className="hover:underline">{listing.contactPhone}</a>
            </div>
          )}
          {listing.contactEmail && (
            <div className="flex items-center gap-2 text-sm">
              <Mail className="h-3 w-3 text-primary shrink-0" />
              <a href={`mailto:${listing.contactEmail}`} className="hover:underline truncate">{listing.contactEmail}</a>
            </div>
          )}
        </div>
    ...
    <div className="container mx-auto px-4 py-16">
      ...
      {listings.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-lg text-muted-foreground mb-6">No approved {title.toLowerCase()} listed yet.</p>
          <Button asChild>
            <Link href="/add-your-listing">Be the first to list your service here</Link>
          </Button>
        </div>
      )}
    </div>
